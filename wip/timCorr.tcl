...
#   ::tb::p2pdelay::rexec ::tb::p2pdelay::stopP2pServer
  ...
  # SERVER
  source timCorr.tcl
  p2pdelay config -port 12345 -part xc7vx415tffg1158-2L -log timCorr.log -nodebug -echo
...
namespace eval ::tb {
    namespace export p2pdelay get_p2p_delay get_p2p_info
}

namespace eval ::tb::p2pdelay {
    namespace export get_p2p_delay get_p2p_info
}
...
eval [list namespace eval ::tb::p2pdelay {
  ...
  array set params [list host {} port {12345} mode {client} log {} verbose 0 part {} timeout 0 debug 0 echo 0 ]
} ]
...
proc ::tb::p2pdelay::p2pdelay { args } {
  ...
  if {$show_help} {
   ...
   Example: Server side
      p2pdelay config -port 12345 -part xc7vx415tffg1158-2L -log timCorr.log -debug -echo
      ::tb::p2pdelay::startP2pServer -timeout 30
...
proc ::tb::p2pdelay::method:reset {args} {
  ...
  set result {}
  set tcpstate {}
  array set params [list host {} port {12345} mode {client} log {} verbose 0 part {} timeout 0 debug 0]
  return -code ok
}
...
proc ::tb::p2pdelay::getP2pInfo { args } {
  ...
  variable params
  set defaults [list -from {} -to {} -port $params(port) -host $params(host)]
...
# if {$params(debug)} { puts "<from:$from><to:$to>" }
  ...
  if {($port == {}) || ($host == {})} {
    set report [getP2pRouteReport -from $from -to $to]
  } else {
    if {[isP2pServerReady -port $port -host $host]} {
      if {$params(debug)} { puts stderr " -D- Sending request to server host:$host / port:$port" }
      set report [rexec [format {::tb::p2pdelay::getP2pRouteReport -from "%s" -to "%s"} $from $to]]
    } else {
...
proc ::tb::p2pdelay::getP2pRouteReport { args } {
  ...
  eval [concat ::internal::route_dbg_p2p_route $args > p2p${pid}.out]
...
#   create_project p2pServer -in_memory -part $part
  ...
  if {$::tb::p2pdelay::tcpstate == {tcp:timeout}} {
    puts " -I- Server stopped by timeout"
  }
  stopP2pServer
...
# if {$params(debug)} { puts "<from:$from><to:$to>" }
  ...
  return [getP2pInfo -from $from -to $to -host $host -port $port]
}
...
proc ::tb::p2pdelay::method:get_p2p_delay {args} {
  ...
  Example:
     ...
     p2pdelay get_p2p_delay -from int1_reg/Q -to {SLICE_X47Y51 A1}
} ]
    # HELP -->
    return -code ok
  }
...
proc ::tb::p2pdelay::method:status {args} {
  ...
  puts " -I- online P2P server host:$host / port:$port / part:[rexec {set ::tb::p2pdelay::params(part)}]"
  return -code ok
}

#################################################################################

namespace import ::tb::p2pdelay
namespace import ::tb::p2pdelay::get_p2p_delay
namespace import ::tb::p2pdelay::get_p2p_info
