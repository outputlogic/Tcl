...
proc routing_overlay {} {
  ...
  set max_tile [lindex [lindex $tile_sort_max 0] 0]
  
  if {$verbose_debug}  {puts "DEBUG: [lindex [lindex $tile_sort_max 0] 0]"}
  if {$verbose_debug}  {puts "DEBUG: [lindex [lindex $tile_sort_max 0] 1]"}
  if {$verbose_debug}  {puts "DEBUG: Highest utilized Node is [lindex $tile_sort_max 0]"}
...
# FROM THE HIGHEST UTILIZED (NODES) TILE DISPLAY A LIST OF NETS THAT TRAVEL THROUGH
# THE TILE
show_objects -name max_nets [get_nets -filter {TYPE == "SIGNAL"} -of_objects [get_pips -of_objects [get_tiles $max_tile]]]
