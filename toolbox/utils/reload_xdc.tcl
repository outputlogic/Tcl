...
## 2014.03.20 - Added support for reports with out-of-context sections
## 2013.10.04 - Changed version format to 2013.10.03 to be compatible with 'package' command
## 09/30/2013 - Added -unmanaged to read_xdc command
## 07/18/2013 - Initial release
########################################################################################

namespace eval ::tb::utils {
    namespace export reload_xdc
}
...
proc ::tb::::utils::reload_xdc {args} {
  return [uplevel [concat ::tb::utils::reload_xdc::main $args]]
}

eval [list namespace eval ::tb::utils::reload_xdc {
  variable version {2015.03.09}
} ]

#------------------------------------------------------------------------
# ::tb::utils::reload_xdc::main
#------------------------------------------------------------------------
# Main function
#------------------------------------------------------------------------
proc ::tb::utils::reload_xdc::main { args } {
  # Summary :
  # Argument Usage:
  # Return Value:
...
#   puts "<filename:$filename>"
  ...
  array set tables [::tb::utils::reload_xdc::extract_tables $report]
...
#------------------------------------------------------------------------
# ::tb::utils::reload_xdc::lshift
#------------------------------------------------------------------------
# **INTERNAL**
#------------------------------------------------------------------------
# Stack function
#------------------------------------------------------------------------
proc ::tb::utils::reload_xdc::lshift {inputlist} {
  # Summary :
  # Argument Usage:
  # Return Value:
...
#------------------------------------------------------------------------
# ::tb::utils::reload_xdc::extract_columns
#------------------------------------------------------------------------
# **INTERNAL**
#------------------------------------------------------------------------
...
#------------------------------------------------------------------------
proc ::tb::utils::reload_xdc::extract_columns { str match } {
  # Summary :
  # Argument Usage:
  # Return Value:
...
#------------------------------------------------------------------------
# ::tb::utils::reload_xdc::extract_row
#------------------------------------------------------------------------
# **INTERNAL**
#------------------------------------------------------------------------
...
# of the columns
#------------------------------------------------------------------------
proc ::tb::utils::reload_xdc::extract_row {str columns} {
  # Summary :
  # Argument Usage:
  # Return Value:
...
#------------------------------------------------------------------------
# ::tb::utils::reload_xdc::extract_tables
#------------------------------------------------------------------------
# **INTERNAL**
#------------------------------------------------------------------------
# Extract the synthesis and implementation tables from the report
#------------------------------------------------------------------------
proc ::tb::utils::reload_xdc::extract_tables {report} {
  # Summary :
  # Argument Usage:
  # Return Value:
  ...
  for {set index 0} {$index < [llength $report]} {incr index} {
    ...
    switch $SM {
      header {
        if {[regexp {^\-+\s+\-+\s+\-+} $line]} {
          set columns [::tb::utils::reload_xdc::extract_columns $line { }]
#           puts "Columns: $columns"
          set header [::tb::utils::reload_xdc::extract_row [lindex $report [expr $index -1]] $columns]
          set row [list]
          foreach elm $header {
...
#           puts "found implementation"
      ...
      table {
        ...
        if {(![regexp {^\s*$} $line]) && (![regexp -nocase {^\s*\<\s*empty\s*\>\s*$} $line])} {
          set row [::tb::utils::reload_xdc::extract_row $line $columns]
          lappend table $row
#           puts "row:$row"
        } else {
          switch $type {
