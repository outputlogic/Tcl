...
#     }
    ...
    if {[info exists CONFIG_VARS(CHECKPOINT)]} {
      # If config file specify a checkpoint name
      set checkpoint $CONFIG_VARS(CHECKPOINT)
    } else {
      # Look for post-place checkpoint
      set checkpoint {postplace.dcp}
      # Look for post-route checkpoint
