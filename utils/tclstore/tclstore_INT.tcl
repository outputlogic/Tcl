...
## 
## Version:        2014.11.05
## Tool Version:   Vivado 2014.3
##
########################################################################################

########################################################################################
## 2014.11.05 - Renamed 'git_pull_request' to 'git_revert_pull'
##            - Renamed 'git_merge_request' to 'git_merge_pull'
##            - Added 'git_patch_pull'
##            - Added 'git_check_patch'
##            - Added '-branch' to 'clone_repo' and 'configure' to support Git branch
##            - Modify 'clone_repo' to only pull the requested branch, not the whole repo
## 2014.10.28 - Renamed 'git_pull_request' to 'git_merge_request'
##            - Added 'git_merge_request'
##            - Added 'git_head'
##            - Created alias 'git'
## 2014.09.09 - Updated for new catalog API for 2014.3
...
# # If this is a new app, use -new_app with update_app_catalog
# tclstore update_app_catalog -catalog 2014.3 -properties { revision_history {} }
# # Check all the files
...
# tclstore update_app_catalog -catalog 2014.3 -properties [list commit_id $commit_id]
# tclstore git status
# tclstore git commit -a -m 'Updated commit id inside 2014.3 catalog'
...
# Create alias: 'git' -> 'tclstore git'
interp alias {} git {} tclstore git
...
#   variable debug 1
  ...
  array set params [list repository {} app {} catalog {} git {/tools/batonroot/rodin/devkits/lnx64/git-1.8.3/bin/git} branch {master} ]
...
proc ::tclapp::tclstore::tclstore { args } {
  ...
  if {[package vcompare [version -short] 2014.3] < 0} {
    error " -E- need Vivado 2014.3 and upward"
  }
  ...
  if {$show_help} {
   ...
   Example: Contributor Flow
      ...
      tclstore update_app_catalog -catalog 2014.3 -revision {Added new command}
      # To create app catalog for new app:
      tclstore update_app_catalog -new_app -catalog 2014.3 -revision {First release} -p [list pkg_require {Vivado 2014.1} summary {This is my new app} company_display {Xilinx, Inc.} display {My New App} ]
      # For GUI testing:
      tclstore update_main_catalog -catalog 2014.3 -app {company::myapp}
      tclstore install_app
      # Check all the files under the app directory ONLY!
   ...
   Example: Gatekeeper Flow
      ...
      tclstore uninstall_app
      tclstore install_app
      tclstore update_main_catalog -catalog 2014.3
      # Check all the files
    } ]
...
#------------------------------------------------------------------------
# ::tclapp::tclstore::getGitRevParse
#------------------------------------------------------------------------
# **INTERNAL**
#------------------------------------------------------------------------
# Get the Git repo commit id returned by 'git rev-parse master' 
#------------------------------------------------------------------------
proc ::tclapp::tclstore::getGitRevParse {} {
  # Summary :
  # Argument Usage:
  # Return Value:
  ...
  set sha1 {}
  if {$verbose} { puts " -I- rev-parse $params(branch)" }
  foreach [list exitCode message] [runGitCmd rev-parse $params(branch)] {break}
...
proc ::tclapp::tclstore::method:configure {args} {
  ...
  Example:
     tclstore configure -repository /home/user/test/tclstore/XilinxTclStore -app xilinx::designutils -catalog 2014.3
...
proc ::tclapp::tclstore::method:clone_repo {args} {
  ...
  if {$params(branch) == {}} {
    ...
    puts " -I- cloning Xilinx GitHub repository (branch $params(branch)) to [file normalize $dir]"
    if {$gitUserID == {}} {
      foreach [list exitCode message] [runGitCmd clone -b $params(branch) --single-branch https://github.com/Xilinx/XilinxTclStore.git] {break}
    } else {
      foreach [list exitCode message] [runGitCmd clone -b $params(branch) --single-branch https://$gitUserID@github.com/Xilinx/XilinxTclStore.git] {break}
    }
  }
  ...
  if {$pullRequest != {}} {
    ::tclapp::tclstore::method:git_merge_pull $pullRequest
  }
...
proc ::tclapp::tclstore::method:git_rev_parse {} {
  ...
  return [::tclapp::tclstore::getGitRevParse]
}

#------------------------------------------------------------------------
# ::tclapp::tclstore::method:git_revert_pull
#------------------------------------------------------------------------
# Usage: tclstore git_revert_pull <pull_request_number>
#------------------------------------------------------------------------
# Revert the Git repo to specific pull request.
#------------------------------------------------------------------------
proc ::tclapp::tclstore::method:git_revert_pull {pullRequest} {
  # Summary :
  # Argument Usage:
  # Return Value:

  # Revert Git repository to pull request
  variable params
  variable verbose
  variable debug
  set error 0
  ...
  if {$pullRequest != {}} {
    puts " -I- commit ID: [::tclapp::tclstore::getGitRevParse]"
    ...
    puts " -I- resetting local repo to pull request refs/pull/$pullRequest/head"
...
#------------------------------------------------------------------------
# ::tclapp::tclstore::method:git_merge_pull
#------------------------------------------------------------------------
# Usage: tclstore git_merge_pull <pull_request_number>
#------------------------------------------------------------------------
...
#------------------------------------------------------------------------
proc ::tclapp::tclstore::method:git_merge_pull {pullRequest} {
  # Summary :
  # Argument Usage:
  # Return Value:
...
#------------------------------------------------------------------------
# ::tclapp::tclstore::method:git_patch_pull
#------------------------------------------------------------------------
# Usage: tclstore git_patch_pull <pull_request_number>
#------------------------------------------------------------------------
# Patch pull request inside repository if applicable. 
#------------------------------------------------------------------------
proc ::tclapp::tclstore::method:git_patch_pull {pullRequest} {
  # Summary :
  # Argument Usage:
  # Return Value:

  # Patch pull request inside Git repository
  variable params
  variable verbose
  variable debug
  set error 0
  ...
  if {$params(repository) == {}} {
    puts " -E- use tclstore config -repository to specify the local repository"
    incr error
  }
  ...
  # Patching with git: curl https://github.com/Xilinx/XilinxTclStore/pull/354.patch -x https://proxy:80 | git am
  #  OR
  # Patching with git: curl https://github.com/Xilinx/XilinxTclStore/pull/354.patch -x https://proxy:80 > 354.patch
  #                    git am < 354.patch

  if {$pullRequest != {}} {
    puts " -I- commit ID: [::tclapp::tclstore::getGitRevParse]"
    
    puts " -I- creating patch for pull request $pullRequest (https://github.com/Xilinx/XilinxTclStore/pull/$pullRequest.patch)"
    foreach [list exitCode message] [system curl https://github.com/Xilinx/XilinxTclStore/pull/$pullRequest.patch -x https://proxy:80 > $params(repository)/$pullRequest.patch ] {break}

    puts " -I- applying patch for pull request $pullRequest"
...
#     foreach [list exitCode message] [system ssh git-dev [format {cd %s ; %s %s < %s} $params(repository) $params(git) {am --signoff} $pullRequest.patch ] ] { break }
    foreach [list exitCode message] [system ssh git-dev [format {cd %s ; %s %s < %s} $params(repository) $params(git) {am} $pullRequest.patch ] ] { break }
    ...
    puts " -W- TODO: delete patch $params(repository)/$pullRequest.patch"
  }
...
#------------------------------------------------------------------------
# ::tclapp::tclstore::method:git_check_patch
#------------------------------------------------------------------------
# Usage: tclstore git_check_patch <pull_request_number>
#------------------------------------------------------------------------
# Check that a patch can be pulled inside inside repository.
#------------------------------------------------------------------------
proc ::tclapp::tclstore::method:git_check_patch {pullRequest} {
  # Summary :
  # Argument Usage:
  # Return Value:

  # Check patch for pull request
  variable params
  variable verbose
  variable debug
  set error 0
  ...
  if {$pullRequest != {}} {
    ...
    puts " -I- checking changes from patch"
    foreach [list exitCode message] [runGitCmd apply --stat $params(repository)/$pullRequest.patch] {break}
    puts " -I- Status: $message"

    puts " -I- testing patch"
    foreach [list exitCode message] [runGitCmd apply --check $params(repository)/$pullRequest.patch] {break}
    puts " -I- Status: $message"
  }
...
#------------------------------------------------------------------------
# ::tclapp::tclstore::method:git_head
#------------------------------------------------------------------------
# Usage: tclstore git_head
#------------------------------------------------------------------------
# Revert Git repo to HEAD.
#------------------------------------------------------------------------
proc ::tclapp::tclstore::method:git_head {args} {
  # Summary :
  # Argument Usage:
  # Return Value:

  # Revert Git repository to HEAD
  variable params
  variable verbose
  variable debug
  set error 0
  ...
  puts " -I- reverting to HEAD (upstream/$params(branch))"
  foreach [list exitCode message] [runGitCmd reset --hard upstream/$params(branch)] {break}
