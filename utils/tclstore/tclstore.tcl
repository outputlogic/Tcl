...
########################################################################################
## 2014.11.07 - Fixed issue in 'update_main_catalog'
## 2014.11.05 - Added '-branch' to 'configure' to support Git branch
## 2014.09.09 - Updated for new catalog API for 2014.3
##            - Renamed update_catalog to update_app_catalog
##            - Added update_main_catalog
...
#------------------------------------------------------------------------
# ::tclapp::tclstore::method:update_app_catalog
#------------------------------------------------------------------------
# Usage: tclstore update_app_catalog [<ListOfAppProperties>]
#------------------------------------------------------------------------
# Update the app catalog XML
#------------------------------------------------------------------------
proc ::tclapp::tclstore::method:update_app_catalog {args} {
  # Summary :
  # Argument Usage:
  # Return Value:

  # Update app catalog XML (-help)
  variable params
  variable verbose
  variable debug
  ...
  while {[llength $args]} {
    ...
    switch -exact -- $name {
      ...
      -r -
      -revision {
           set xmlproperties [concat $xmlproperties [list {revision_history} [lshift args]] ]
      }
      -n -
      -new -
      -new_app {
  ...
  if {$help} {
    puts [format {
  Usage: tclstore update_app_catalog
              [-p <xml_properties>|-prop <xml_properties>|-properties <xml_properties>]
              [-r <revision_history>|-revision <revision_history>]
              [-c <catalog>|-catalog <catalog>]
              [-update_main|-update_main_catalog]
              [-n|-new|-new_app]
              [-shallow]
              [-help|-h]

  Description: Update the App XML Catalog
    ...
    Both '-revision <revision_history>' and '-properties revision_history <revision_history>' are equivalent to set the revision history
    
    Use -update_main_catalog to update the main XML catalog for GUI testing
    
    Use -shallow to exclude proc information from app catalog (uncommon usage)

  Example:
     tclstore update_app_catalog -catalog 2014.3
     tclstore update_app_catalog -revision {Added new command}
     tclstore update_app_catalog -revision {Added new command} -update_main_catalog
     tclstore update_app_catalog -new_app -revision {First release} -p [list pkg_require {Vivado 2014.1} summary {This is my new app} company_display {Xilinx, Inc.} display {My New App} ]
} ]
    # HELP -->
    return -code ok
  }
...
#     array set properties [list revision_history {} ]
    ...
    foreach prop {name display company company_display summary author pkg_require revision revision_history commit_id} {
      if {[info exists properties($prop)]} {
        ...
        tclapp::add_app_property $prop $properties($prop)
      }
    }
...
#     }
    puts " -I- saving app catalog for release $catalog"
    if {$updateMainCatalog} {
      tclapp::save_app_catalog -update_main_catalog -release $catalog
    } else {
      tclapp::save_app_catalog -release $catalog
    }

  } else {
    # Update XML for existing app
    array set properties $xmlproperties
    if {![info exists properties(revision_history)]} {
      error " -E- no revision history provided"
    }

    puts " -I- updating app catalog for '$app' ($catalog)"
    if {$updateMainCatalog} {
      if {$shallow} {
        tclapp::update_app_catalog -shallow -update_main_catalog -release $catalog $app $properties(revision_history)
      } else {
        tclapp::update_app_catalog -update_main_catalog -release $catalog $app $properties(revision_history)
      }
    } else {
      if {$shallow} {
        tclapp::update_app_catalog -shallow -release $catalog $app $properties(revision_history)
      } else {
        tclapp::update_app_catalog -release $catalog $app $properties(revision_history)
      }
    }
...
#------------------------------------------------------------------------
# ::tclapp::tclstore::method:update_main_catalog
#------------------------------------------------------------------------
# Usage: tclstore update_main_catalog [<ListOfAppProperties>]
#------------------------------------------------------------------------
# Update the release catalog XML
#------------------------------------------------------------------------
proc ::tclapp::tclstore::method:update_main_catalog {args} {
  # Summary :
  # Argument Usage:
  # Return Value:

  # Update release catalog XML (-help)
  variable params
  variable verbose
  variable debug
  ...
  while {[llength $args]} {
    ...
    switch -exact -- $name {
      ...
      -app -
      -apps {
           set listApps [concat $listApps [lshift args]]
      }
      -h -
      -help {
           set help 1
  ...
  if {$help} {
    puts [format {
  Usage: tclstore update_main_catalog
              [-c <catalog>|-catalog <catalog>]
              [-apps <list_apps>]
              [-delete]
              [-help|-h]

  Description: Update the Release XML Catalog

  Example:
     tclstore update_main_catalog -catalog 2014.3
     tclstore update_main_catalog -catalog 2014.3 -apps {xilinx::designutils xilinx::diff}
     tclstore update_main_catalog -catalog 2014.3 -apps {xilinx::designutils xilinx::diff} -delete
} ]
    # HELP -->
    return -code ok
  }
  ...
  switch -glob -- [version -short] {
    2014.3* {
      puts " -I- updating release catalog for '$catalog'"
      if {$deleteApps} {
        eval [concat tclapp::update_catalog -delete -release $catalog $listApps]
      } else {
        eval [concat tclapp::update_catalog -release $catalog $listApps]
      }
    }
    default {
      puts " -I- updating release catalog for '$catalog'"
...
if {[file exists [file join [file dirname [info script]] tclstore_INT.tcl]]} {
  puts "  Sourcing [file join [file dirname [info script]] tclstore_INT.tcl]"
  source -notrace [file join [file dirname [info script]] tclstore_INT.tcl]
}
