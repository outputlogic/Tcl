...
# This script reports statistics regarding the properties IS_CLOCK IS_ENABLE IS_PRESET IS_RESET IS_CLEAR IS_SETRESET IS_SET IS_WRITE_ENABLE
...
if {$rebuildCache} {
  ...
  foreach p [list is_clock is_enable is_write_enable is_set is_preset is_reset is_clear is_setreset ] {
...
$tbl header {cell pin clock set preset  reset  clear set/reset enable write_enable name}
...
foreach c $cells cell_ref [get_property REF_NAME $cells] {
  ...
  foreach m $pins ref_pin [get_property REF_PIN_NAME $pins] {
    ...
    lappend row $ref_pin
    foreach p [list is_clock is_set is_preset is_reset is_clear is_setreset is_enable is_write_enable] {
...
foreach p [list is_clock is_set is_preset is_reset is_clear is_setreset is_enable is_write_enable ] {
  ...
  if {$saveTableCSV} {
    $tblCells export -file pinstats2.$p.by_cellref.csv -format csv
    $tblPins export -file pinstats2.$p.by_pinref.csv -format csv
  }
...
if {$saveTableCSV} {
  $tblProps export -format csv -file pinstats2.by_pinref.csv
}
if {$destroyTables} { catch {$tblProps destroy} }

set tblCells [::tb::prettyTable "Properties per Cell Ref"]
$tblCells header {{Cell Ref} {Pin Ref} {Dir} {Pin Properties} {Pin Lib Properties}}
$tblCells config -indent 2
set prevCellRef {}
foreach el [lsort -dictionary [array names allcellrefs]] {
  ...
  $tblCells addrow [list $cell_ref $pin_ref [get_property -quiet {DIRECTION} $libpin] $props $libprops]
  set prevCellRef $cell_ref
}
puts $channel [$tblCells print]
if {$saveTableCSV} {
  $tblCells export -format csv -file pinstats2.by_cellref.csv
}
